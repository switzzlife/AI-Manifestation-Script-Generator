...
def record_voice():
    ...
    if not script_id:
        logging.error("No script_id provided in the form data")
    ...
    if audio_file and script_id:
        try:
            ...
            if file_size > 10 * 1024 * 1024:
                logging.error(f"File size ({file_size} bytes) exceeds the limit of 10MB")
            ...
            if file_extension not in allowed_extensions:
                logging.error(f"Invalid file type: {file_extension}. Allowed types: {', '.join(allowed_extensions)}")
                return jsonify({'error': f"Invalid file type. Allowed types: {', '.join(allowed_extensions)}"}), 400
            
            script = Script.query.get(script_id)
            if script and script.user_id == current_user.id:
                filename = secure_filename(f"user_voice_{current_user.id}_{script_id}_{datetime.now().strftime('%Y%m%d%H%M%S')}.{file_extension}")
